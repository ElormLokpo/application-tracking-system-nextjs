...
export const googleAuthService = async (email: string, name: string) => {
    ...
    let userCreated;
    if (!user) {
        userCreated = await UserModel.create({ email, fullname: name, isVerified: true });

    }

    const userReponse: UserResponse = {
        id: !user ? userCreated?._id as unknown : user._id as unknown,
        fullname: !user ? userCreated?.fullname as string : user.fullname as string,
        email: !user ? userCreated?.email as string : user.email as string,
        role: !user ? userCreated?.role as Role : user.role as Role
    }
    return await generateToken(userReponse);
}
