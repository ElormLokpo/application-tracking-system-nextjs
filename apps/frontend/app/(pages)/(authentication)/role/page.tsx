...
const RoleCard = ({
  ...
  const defaultStyle =
    "w-[auto] hover:dark:bg-stone-800 hover:cursor-pointer transition-border duration-400 ease-out hover:bg-stone-100 flex items-center gap-2 flex-col border-stone-900  dark:border-stone-200 p-5 rounded-md";
  return (
    <button
      type="button"
      onClick={() => setCurrentRole(roleValue)}
      className={`${currentRole === roleValue ? `${defaultStyle} border-3` : `${defaultStyle} border-1`}`}
    >
      <div>{icon}</div>
      <Typography
        size={"sm"}
